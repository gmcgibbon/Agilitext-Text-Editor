...
public class AgilitextTextArea extends JTextArea
{
	...
	public AgilitextTextArea()
	{
		...
		this.getDocument().addUndoableEditListener(undoMan);
	...
	/**
	 * doUndo: Used to attempt an undo operation on previous action
	 * @return: true if operation was successful, false if it was not
	 */
	protected boolean doUndo()
	{
		//initialize undone boolean to false
		boolean undone=false;
		
		try
    	{
			//if undoMan can undo operation
    		if(undoMan.canUndo())
    		{
    			//call undo
    			undoMan.undo();
	...
	/**
	 * doRedo: Used to attempt an redo operation on previous action
	 * @return: true if operation was successful, false if it was not
	 */
	protected boolean doRedo()
	{
		//initialize redone to false
		boolean redone=false;
		
		try
    	{
			//if undoMan can redo operation
    		if(undoMan.canRedo())
    		{
    			//call redo
    			undoMan.redo();
	...
	/**
	 * insertDate: Used to insert current date into text area
	 */
	protected void insertDate()
	{
		//insert current date on current caret position
		insert(Appsistant.getCurrentDate(), getCaretPosition());
	}
	
	/**
	 * insertTime: Used to insert current time into text area
	 */
	protected void insertTime()
	{
		//insert current time on current caret position
		insert(Appsistant.getCurrentTime(), getCaretPosition());
	}
